...
class Solution {
    ...
    public void helper(TreeNode root) {
        if (root == null)
            return;
        // asks root's right child to bring the right sum
        helper(root.right);
        // update the new sum and set root's value to the sum
        root.val = sum + root.val;
        sum = root.val;
        // same thing to be done for the root's left child and sum is passed along
        // since it's left is less than the sum we have calculated for root
        helper(root.left);
    }
